...
from langgraph.checkpoint.aredis import AsyncRedisSaver
...
async def test_invoke_checkpoint_aredis(mocker: MockerFixture) -> None:
    add_one = mocker.Mock(side_effect=lambda x: x["total"] + x["input"])
    ...
    async with AsyncRedisSaver.from_url("redis://localhost:6379/2") as memory:
